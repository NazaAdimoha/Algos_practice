...
function classPhotos(blueShirtHeights,redShirtHeights) {
    ...
    let sortedRedShirtHeights = redShirtHeights.sort((x,y) => x - y);
    let red = [];
    let blue = [];
    if(sortedBlueShirtHeights){
  //     blue.push(sortedBlueShirtHeights);
      return true
    } else if(sortedRedShirtHeights){
  //     red.push(sortedRedShirtHeights);
     return true
    }else {
      return false;
    }
